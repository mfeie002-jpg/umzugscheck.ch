  ...
  <section className="py-12">
    <div className="container mx-auto px-4">
      <div className="grid md:grid-cols-2 gap-8 items-center max-w-5xl mx-auto">
        <div>
          <div className="inline-flex items-center gap-2 bg-secondary/10 text-secondary px-3 py-1 rounded-full text-sm font-bold mb-4">
            <TrendingDown className="h-4 w-4" />
            Bis 40% präziser – bis 40% sparen
          </div>
          <h2 className="text-2xl md:text-3xl font-bold mb-4">KI Video-Analyse: Präzisere Offerten = Weniger Kosten</h2>
          <p className="text-muted-foreground mb-6">
            Durch exakte KI-Analyse Ihres Inventars erhalten Sie <strong>genauere Offerten</strong> – 
            keine übertriebenen Sicherheitszuschläge, kein Rätselraten. Das spart Ihnen bares Geld.
          </p>
          <ul className="space-y-3 mb-6">
            {[
              { icon: Camera, text: "30 Sek. Video aufnehmen", highlight: "Einfach mit Handy" },
              { icon: Sparkles, text: "KI analysiert jeden Gegenstand", highlight: "Exaktes Volumen" },
              { icon: TrendingDown, text: "Präzise Offerte = faire Preise", highlight: "Bis 40% sparen" },
            ].map((item, i) => (
              <li key={i} className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-secondary font-semibold">{item.highlight}</span>
                </div>
              </li>
            ))}
          </ul>
          <Link to="/rechner/video">
            <Button size="lg" className="bg-secondary hover:bg-secondary/90 text-secondary-foreground shadow-lg shadow-secondary/25">
              <Play className="mr-2 h-5 w-5" />
              Video-Schätzung starten & sparen
            </Button>
          </Link>
        </div>
