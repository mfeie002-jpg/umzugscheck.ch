/**
 * V8a Feedback-Based - Decision-Free Optimized
 * 
 * ChatGPT Pro Feedback implementiert:
 * - Transparenz-Panel: "Was passiert als Nächstes?"
 * - Review Step (Kontrolle) + editierbar
 * - Eingaben minimieren (wirklich kurz)
 * - Default-Optionen + "Wir wählen für dich"
 */

import React, { useState } from 'react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Progress } from '@/components/ui/progress';
import { Shield, CheckCircle2, Lock, MapPin, User, Mail, Phone, Edit2, ArrowRight, Info } from 'lucide-react';

const STEPS = [
  { id: 1, title: 'Basics' },
  { id: 2, title: 'Überprüfen' },
  { id: 3, title: 'Fertig' },
];

function ProgressHeader({ step, total, title }: { step: number; total: number; title: string }) {
  const pct = Math.round((step / total) * 100);
  return (
    <div className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b pb-4 pt-4 px-4">
      <div className="max-w-md mx-auto">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold">{title}</h2>
          <span className="text-sm text-muted-foreground">Schritt {step}/{total}</span>
        </div>
        <Progress value={pct} className="h-2" />
      </div>
    </div>
  );
}

function TransparencyPanel() {
  return (
    <div className="bg-muted/50 border rounded-xl p-4 mb-6">
      <div className="flex items-start gap-3">
        <Info className="h-5 w-5 text-primary mt-0.5" />
        <div className="space-y-2 text-sm">
          <p className="font-semibold">Was passiert als Nächstes?</p>
          <ol className="space-y-1 text-muted-foreground list-decimal list-inside">
            <li>Du gibst nur PLZ + Kontakt an</li>
            <li>Wir matchen dich mit passenden Firmen</li>
            <li>Du erhältst 3-5 unverbindliche Offerten</li>
          </ol>
          <p className="text-xs flex items-center gap-1">
            <Lock className="h-3 w-3" />
            Deine Daten werden nur für Offerten verwendet
          </p>
        </div>
      </div>
    </div>
  );
}

function TrustBar() {
  return (
    <div className="flex flex-wrap justify-center gap-4 text-sm mb-6">
      <div className="flex items-center gap-2">
        <Shield className="h-4 w-4 text-primary" />
        <span className="font-medium">Unverbindlich</span>
      </div>
      <div className="flex items-center gap-2">
        <CheckCircle2 className="h-4 w-4 text-primary" />
        <span className="font-medium">Geprüft</span>
      </div>
      <div className="flex items-center gap-2">
        <Lock className="h-4 w-4 text-primary" />
        <span className="font-medium">DSG/DSGVO</span>
      </div>
    </div>
  );
}

function StickyFooterCTA({
  primaryLabel, onPrimary, disabled, hint, secondaryLabel, onSecondary,
}: {
  primaryLabel: string; onPrimary: () => void; disabled?: boolean; hint?: string;
  secondaryLabel?: string; onSecondary?: () => void;
}) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 bg-background/95 backdrop-blur border-t safe-area-inset-bottom">
      <div className="max-w-md mx-auto p-4 space-y-2">
        {hint && <p className="text-center text-sm text-muted-foreground">{hint}</p>}
        <Button onClick={onPrimary} disabled={disabled} className="w-full h-14 text-lg font-semibold" size="lg">
          {primaryLabel}
        </Button>
        {secondaryLabel && onSecondary && (
          <Button variant="ghost" onClick={onSecondary} className="w-full">{secondaryLabel}</Button>
        )}
      </div>
    </div>
  );
}

function Field({
  label, name, type = 'text', value, onChange, placeholder, error, autoComplete, inputMode, icon: Icon,
}: {
  label: string; name: string; type?: string; value: string; onChange: (v: string) => void;
  placeholder?: string; error?: string; autoComplete?: string; inputMode?: 'text' | 'numeric' | 'tel' | 'email';
  icon?: React.ElementType;
}) {
  return (
    <div className="space-y-2">
      <Label htmlFor={name} className="text-base font-medium">{label}</Label>
      <div className="relative">
        {Icon && <Icon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />}
        <Input
          id={name} name={name} type={type} value={value} onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder} autoComplete={autoComplete} inputMode={inputMode}
          className={`h-12 text-base ${Icon ? 'pl-10' : ''} ${error ? 'border-destructive' : ''}`}
        />
      </div>
      {error && <p className="text-sm text-destructive">{error}</p>}
    </div>
  );
}

function ReviewCard({
  items,
  onEdit,
}: {
  items: { label: string; value: string }[];
  onEdit: () => void;
}) {
  return (
    <div className="bg-muted/30 rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">Deine Angaben</h3>
        <Button variant="ghost" size="sm" onClick={onEdit} className="h-8 text-primary">
          <Edit2 className="h-4 w-4 mr-1" />
          Bearbeiten
        </Button>
      </div>
      <div className="space-y-3">
        {items.map((it) => (
          <div key={it.label} className="flex justify-between text-sm">
            <span className="text-muted-foreground">{it.label}</span>
            <span className="font-medium">{it.value || '—'}</span>
          </div>
        ))}
      </div>
      <p className="text-xs text-muted-foreground pt-2 border-t">
        Wir teilen deine Daten nur mit passenden, geprüften Umzugsfirmen zur Offertstellung.
      </p>
    </div>
  );
}

export const V8aFeedbackBased: React.FC = () => {
  const [currentStep, setCurrentStep] = useState(1);
  const [formData, setFormData] = useState({
    fromZip: '', toZip: '', name: '', email: '', phone: '',
  });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [isSubmitted, setIsSubmitted] = useState(false);

  const updateField = (field: string, value: string) => {
    setFormData(prev => ({ ...prev, [field]: value }));
    if (errors[field]) setErrors(prev => ({ ...prev, [field]: '' }));
  };

  const validateStep1 = () => {
    const newErrors: Record<string, string> = {};
    if (formData.fromZip.length < 4) newErrors.fromZip = 'Gültige PLZ eingeben';
    if (formData.toZip.length < 4) newErrors.toZip = 'Gültige PLZ eingeben';
    if (formData.name.trim().length < 2) newErrors.name = 'Name eingeben';
    if (!formData.email.includes('@')) newErrors.email = 'Gültige E-Mail eingeben';
    if (formData.phone.length < 8) newErrors.phone = 'Gültige Telefonnummer eingeben';
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  const handleNext = () => {
    if (currentStep === 1 && validateStep1()) setCurrentStep(2);
    else if (currentStep === 2) setIsSubmitted(true);
  };

  const handleBack = () => {
    if (currentStep > 1) setCurrentStep(currentStep - 1);
  };

  if (isSubmitted) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto">
              <CheckCircle2 className="h-8 w-8 text-primary" />
            </div>
            <h2 className="text-2xl font-bold">Anfrage gesendet!</h2>
            <p className="text-muted-foreground">
              Wir haben passende Firmen für dich ausgewählt. Du erhältst in Kürze 3-5 unverbindliche Offerten.
            </p>
            <div className="bg-muted/50 rounded-xl p-4 text-left text-sm space-y-2">
              <p className="font-semibold">Was passiert jetzt?</p>
              <ul className="space-y-1 text-muted-foreground">
                <li className="flex items-center gap-2">
                  <ArrowRight className="h-3 w-3" />
                  Firmen prüfen deine Anfrage
                </li>
                <li className="flex items-center gap-2">
                  <ArrowRight className="h-3 w-3" />
                  Du erhältst Offerten per E-Mail
                </li>
                <li className="flex items-center gap-2">
                  <ArrowRight className="h-3 w-3" />
                  Vergleiche und wähle die beste
                </li>
              </ul>
            </div>
          </CardContent>
        </Card>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-background pb-32">
      <ProgressHeader step={currentStep} total={STEPS.length - 1} title={STEPS[currentStep - 1].title} />

      <div className="max-w-md mx-auto p-4 pt-6">
        {currentStep === 1 && (
          <>
            <TransparencyPanel />
            
            <Card>
              <CardContent className="pt-6 space-y-6">
                <div className="text-center mb-4">
                  <h1 className="text-xl font-bold mb-2">Nur 5 Felder – fertig!</h1>
                  <p className="text-sm text-muted-foreground">
                    Wir wählen automatisch die besten Firmen für dich
                  </p>
                </div>

                <Field label="Von PLZ" name="fromZip" value={formData.fromZip}
                  onChange={(v) => updateField('fromZip', v)} placeholder="z.B. 8001"
                  inputMode="numeric" autoComplete="postal-code" error={errors.fromZip} icon={MapPin} />

                <Field label="Nach PLZ" name="toZip" value={formData.toZip}
                  onChange={(v) => updateField('toZip', v)} placeholder="z.B. 3011"
                  inputMode="numeric" autoComplete="postal-code" error={errors.toZip} icon={MapPin} />

                <Field label="Name" name="name" value={formData.name}
                  onChange={(v) => updateField('name', v)} placeholder="Vor- und Nachname"
                  autoComplete="name" error={errors.name} icon={User} />

                <Field label="E-Mail" name="email" type="email" value={formData.email}
                  onChange={(v) => updateField('email', v)} placeholder="ihre@email.ch"
                  autoComplete="email" inputMode="email" error={errors.email} icon={Mail} />

                <Field label="Telefon" name="phone" type="tel" value={formData.phone}
                  onChange={(v) => updateField('phone', v)} placeholder="079 123 45 67"
                  autoComplete="tel" inputMode="tel" error={errors.phone} icon={Phone} />
              </CardContent>
            </Card>
          </>
        )}

        {currentStep === 2 && (
          <>
            <TrustBar />
            
            <Card>
              <CardContent className="pt-6 space-y-6">
                <div className="text-center mb-4">
                  <h1 className="text-xl font-bold mb-2">Alles korrekt?</h1>
                  <p className="text-sm text-muted-foreground">
                    Überprüfe deine Angaben vor dem Absenden
                  </p>
                </div>

                <ReviewCard
                  items={[
                    { label: 'Von', value: formData.fromZip },
                    { label: 'Nach', value: formData.toZip },
                    { label: 'Name', value: formData.name },
                    { label: 'E-Mail', value: formData.email },
                    { label: 'Telefon', value: formData.phone },
                  ]}
                  onEdit={() => setCurrentStep(1)}
                />

                <div className="bg-primary/5 rounded-xl p-4 text-center">
                  <p className="text-sm font-medium">
                    ✓ Wir wählen 3-5 passende Firmen für dich aus
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Basierend auf deiner Route und Verfügbarkeit
                  </p>
                </div>
              </CardContent>
            </Card>
          </>
        )}
      </div>

      <StickyFooterCTA
        primaryLabel={currentStep === 2 ? 'Offerten erhalten' : 'Überprüfen'}
        onPrimary={handleNext}
        disabled={currentStep === 1 ? (
          formData.fromZip.length < 4 || 
          formData.toZip.length < 4 || 
          formData.name.length < 2 || 
          !formData.email.includes('@') || 
          formData.phone.length < 8
        ) : false}
        hint="Unverbindlich • Kostenlos • Wir wählen für dich"
        secondaryLabel={currentStep > 1 ? 'Zurück' : undefined}
        onSecondary={currentStep > 1 ? handleBack : undefined}
      />
    </div>
  );
};

export default V8aFeedbackBased;
