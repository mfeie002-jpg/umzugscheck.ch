...
interface CantonChecklistProps {
  cantonName: string;
  specialTip?: string;
}
...
const checklistItems = [
  ...
  { icon: FileText, text: "Parkbewilligung beantragen", done: false, tip: "2 Wochen vorher" },
...
export const CantonChecklist = ({ cantonName, specialTip }: CantonChecklistProps) => (
  <Card className="border-2 border-accent/20">
    <CardContent className="p-6">
      <h3 className="font-semibold text-lg mb-4">Umzugscheckliste {cantonName}</h3>
      ...
      {specialTip && (
        <p className="text-xs text-muted-foreground mt-4 text-center">{specialTip}</p>
      )}
    </CardContent>
  </Card>
);
