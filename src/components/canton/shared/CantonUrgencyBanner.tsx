...
interface CantonUrgencyBannerProps {
  cantonName: string;
  discountPercent?: number;
}

export const CantonUrgencyBanner = ({ cantonName, discountPercent = 15 }: CantonUrgencyBannerProps) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveUsers((prev) => Math.max(8, prev + Math.floor(Math.random() * 3) - 1));
    }, 5000);
    ...
    >
      <div className="container mx-auto flex flex-wrap items-center justify-center gap-4 md:gap-8 text-sm">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <span><strong>{discountPercent}%</strong> Rabatt bei Buchung heute</span>
        </div>
        <div className="flex items-center gap-2">
