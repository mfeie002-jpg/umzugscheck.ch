/**
 * V1.d MultiStepCalculator - Optimized based on ChatGPT UX Audit
 * 
 * Key Improvements:
 * 1. ✅ Progress bug fixed (Step 1 = "Schritt 1 von 4")
 * 2. Persistent sticky CTA (always visible with disabled state)
 * 3. Flexible date option (already present, enhanced)
 * 4. Simplified 3-package service model (no dual selection)
 * 5. Step 3 optional with quick CTA at top
 * 6. Sponsored listings more transparent
 * 7. "Both" option in green (not red)
 * 8. Privacy checkbox default UNchecked
 * 9. Distraction-free funnel header
 * 10. KI Video as post-submit upsell
 */
...
import { MoveTypeInitialStepV1d } from "./MoveTypeInitialStepV1d";
import { ServicePackageSelector } from "./ServicePackageSelector";
import { SubmitOptionsCardV1d } from "./SubmitOptionsCardV1d";
...
const FORM_STORAGE_KEY = "umzugscheck_form_data_v1d";
...
export const MultiStepCalculatorV1d = memo(function MultiStepCalculatorV1d() {
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          email: parsed.email || "",
          phone: parsed.phone || "",
          privacyAccepted: false, // Always false on load (Audit #8)
          selectedCompanies: [],
  ...
  // Service ID mapping for companies
  ...
  // Auto-scroll to contact fields after submit option selection (Audit #7)
  useEffect(() => {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-premium overflow-hidden max-w-full overflow-x-hidden">
      {/* Distraction-Free Header (Audit #9) */}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <ValidatedInput
                  ...
                  placeholder="PLZ Ort"
                  list="to-suggestions-v1d"
                />
              </div>
              <datalist id="from-suggestions-v1d">
                {swissPostalCodes.map((p) => (
                  <option key={`from-${p.code}`} value={`${p.code} ${p.city}`} />
                ))}
              </datalist>
              <datalist id="to-suggestions-v1d">
                {swissPostalCodes.map((p) => (
              ...
              {/* Move Date (Audit #3: Flexible option) */}
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2 mb-2">
                  <Checkbox
                    ...
                    className="h-4 w-4"
                  />
                  <label htmlFor="date-flexible-v1d" className="text-[12px] text-muted-foreground cursor-pointer">
              ...
              {/* Service Package Selector (Audit #4) */}
              <ServicePackageSelector
                selectedPackage={formData.selectedPackage}
                selectedAddOns={formData.selectedAddOns}
                onPackageChange={(pkgId, services) => updateFormData("selectedPackage", pkgId)}
                onAddOnToggle={toggleAddOn}
              />
            </motion.div>
          )}

          {/* Step 3: Company Selection (Audit #5: Optional with quick CTA) */}
          {currentStep === 3 && (
            <motion.div
              key="step3"
              variants={stepVariants}
          ...
          {currentStep === 4 && (
            ...
            >
              ...
              <div ref={contactFieldsRef} className="space-y-3">
                ...
                {/* Privacy Checkbox - default UNchecked (Audit #8) */}
                <div className="flex items-start gap-2.5">
                  ...
                  />
                  <label htmlFor="privacy-v1d" className="text-[11px] text-muted-foreground cursor-pointer leading-relaxed">
                ...
                <div className="flex items-start gap-2 mt-2 p-2 rounded-lg bg-green-50 dark:bg-green-950/30">
      ...
      {/* Trust Footer */}
      <div className="hidden md:block bg-muted/30 px-6 py-2.5 border-t border-border">
      ...
      <CaptureReadySentinel 
        ...
        flow="v1d" 
        isReady={true}
        metadata={{ variant: "v1d-audit-optimized" }}
      />
    </div>
  );
});
