...
import { 
  ...
  Sparkles, Camera, Shield, Clock, ChevronDown
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";

interface LocationHeroEnhancedProps {
  // Core
  variant: 'canton' | 'city';
  locationName: string;
  ...
  // Canton-specific: cities for dropdown
  citiesInCanton?: Array<{ name: string; slug: string }>;
  
  // City-specific: quartiere chips
  quartiere?: string[];
  
  // Canton reference for city
  cantonSlug?: string;
  cantonName?: string;
}
...
export const LocationHeroEnhanced = ({
  variant,
  locationName,
  locationShort,
  h1,
  ...
  const [showGemeindeDropdown, setShowGemeindeDropdown] = useState(false);
  ...
  useEffect(() => {
    if (moveDirection === 'within') {
      setFromValue(locationName);
      setToValue('');
    } else if (moveDirection === 'to') {
      setFromValue('');
      setToValue(locationName);
    } else if (moveDirection === 'from') {
      setFromValue(locationName);
      setToValue('');
    }
  }, [moveDirection, locationName]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    params.set('placeContext', locationName);
    params.set('pageType', variant);
  ...
  const directionLabels = variant === 'canton' 
    ? {
        within: `Innerhalb Kanton ${locationName}`,
        to: `Nach ${locationName}`,
        from: `Von ${locationName}`,
      }
    : {
        within: `Innerhalb ${locationName}`,
        to: `Nach ${locationName}`,
        from: `Von ${locationName}`,
      };
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        {backgroundImage ? (
          <img 
            src={backgroundImage}
            alt={`Umzug ${variant === 'canton' ? 'im Kanton' : 'in'} ${locationName}`}
      ...
      <div className="container mx-auto px-4 relative z-10 py-8 md:py-16">
        ...
        >
          ...
          {variant === 'canton' && moveDirection === 'within' && citiesInCanton.length > 0 && (
            <div className="mb-4">
          ...
          {/* City: Quartier Chips */}
          {variant === 'city' && quartiere.length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-muted-foreground mb-2">Quartiere in {locationName}:</p>
              <div className="flex flex-wrap gap-2">
          ...
          <div className="text-center mt-4">
            ...
            >
              Preise {variant === 'canton' ? 'im Kanton' : 'in'} {locationName} ansehen
