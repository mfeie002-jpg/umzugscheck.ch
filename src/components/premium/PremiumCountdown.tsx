...
export const PremiumCountdown = () => {
  ...
  const TimeBlock = ({ value, label }: { value: number; label: string }) => (
    <div className="text-center">
      <div className="bg-card rounded-xl p-3 md:p-4 shadow-lg border min-w-[60px] md:min-w-[80px]">
        <span className="text-2xl md:text-4xl font-bold text-primary">
          {String(value).padStart(2, "0")}
        </span>
      </div>
      <span className="text-xs md:text-sm text-muted-foreground mt-2 block">{label}</span>
    </div>
  );
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <div className="flex justify-center gap-3 md:gap-4 mb-8">
            ...
            <div className="text-2xl md:text-4xl font-bold text-muted-foreground self-start pt-3 md:pt-4">:</div>
            <TimeBlock value={timeLeft.hours} label="Stunden" />
            <div className="text-2xl md:text-4xl font-bold text-muted-foreground self-start pt-3 md:pt-4">:</div>
            <TimeBlock value={timeLeft.minutes} label="Minuten" />
            <div className="hidden md:block text-2xl md:text-4xl font-bold text-muted-foreground self-start pt-3 md:pt-4">:</div>
