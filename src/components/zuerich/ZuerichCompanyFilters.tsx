...
import { Filter, Star, Clock, TrendingDown, SlidersHorizontal, X } from "lucide-react";
...
interface ZuerichCompanyFiltersProps {
...
export const ZuerichCompanyFilters = ({ onFilterChange }: ZuerichCompanyFiltersProps) => {
  ...
  const activeFiltersCount =
    filters.priceLevel.length + filters.services.length + (filters.minRating > 0 ? 1 : 0) + (filters.availability ? 1 : 0);

  const clearFilters = () => {
    const newFilters = { priceLevel: [], minRating: 0, availability: false, services: [] };
    setFilters(newFilters);
    onFilterChange?.(newFilters);
  };
    ...
    <div className="mb-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          ...
          <Button
            variant={filters.priceLevel.includes("günstig") ? "default" : "outline"}
            size="sm"
            onClick={() => togglePriceLevel("günstig")}
            className="gap-1 whitespace-nowrap"
          >
      ...
      {isExpanded && (
        ...
        >
          <div>
            <label className="text-sm font-medium mb-2 block">Preisniveau</label>
          ...
          <div>
            <label className="text-sm font-medium mb-2 block">Mindestbewertung: {filters.minRating || "Alle"}</label>
            <Slider
