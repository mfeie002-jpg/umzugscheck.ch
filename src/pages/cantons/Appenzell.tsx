...
const appenzellCities = ["Herisau", "Appenzell", "Teufen", "Speicher", "Bühler", "Gais", "Urnäsch", "Hundwil", "Schwellbrunn", "Waldstatt"];

const topCompanies = [
  { name: "Appenzeller Umzüge AG", rating: 4.9, reviews: 112, priceLevel: "fair", sponsored: true, available: true, badge: "Top bewertet" },
  { name: "Herisau Moving", rating: 4.8, reviews: 94, priceLevel: "günstig", sponsored: true, available: true, badge: "Preistipp" },
  { name: "Säntis Express", rating: 4.8, reviews: 78, priceLevel: "fair", sponsored: false, available: true, badge: "Bergexperte" },
  { name: "Teufen Transporte", rating: 4.7, reviews: 64, priceLevel: "günstig", sponsored: false, available: true, badge: "Lokal" },
  { name: "Appenzell Zügeldienst", rating: 4.7, reviews: 56, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Ausserrhoder Moving", rating: 4.6, reviews: 48, priceLevel: "günstig", sponsored: false, available: true, badge: null },
  { name: "Innerrhoder Umzüge", rating: 4.6, reviews: 42, priceLevel: "premium", sponsored: false, available: true, badge: "Tradition" },
  { name: "Gais Transporte", rating: 4.5, reviews: 36, priceLevel: "fair", sponsored: false, available: true, badge: null },
  { name: "Urnäsch Express", rating: 4.5, reviews: 28, priceLevel: "günstig", sponsored: false, available: false, badge: null },
  { name: "Hügelland Moving", rating: 4.4, reviews: 22, priceLevel: "fair", sponsored: false, available: true, badge: null },
];

const priceExamples = [
  { size: "1-2 Zimmer", range: "CHF 410 - 780", avg: "CHF 570" },
  { size: "3-4 Zimmer", range: "CHF 860 - 1'480", avg: "CHF 1'080" },
  { size: "5+ Zimmer", range: "CHF 1'680 - 2'900", avg: "CHF 2'150" },
];
...
const usps = [
  { title: "Appenzellerland-Experten", desc: "Lokale Firmen kennen die Hügellandschaft", icon: Mountain },
  { title: "Bis 40% sparen", desc: "Vergleichen Sie Preise lokaler Appenzeller Firmen", icon: TrendingUp },
...
const faqs = [
  { question: "Was kostet ein Umzug im Appenzell?", answer: "Ein Umzug im Appenzellerland kostet zwischen CHF 410 und CHF 2'900 je nach Wohnungsgrösse." },
  { question: "Gibt es Unterschiede zwischen AR und AI?", answer: "Unsere Partner decken beide Halbkantone (Ausserrhoden und Innerrhoden) ab." },
  { question: "Wie buche ich einen Umzug im Appenzell?", answer: "Füllen Sie unser Formular in 2 Minuten aus und erhalten Sie bis zu 5 kostenlose Offerten." },
  ...
  { question: "Bieten Sie auch Lagerung an?", answer: "Ja, viele unserer Partner bieten Einlagerungsmöglichkeiten im Appenzellerland an." },
];

const jsonLd = {
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  "name": "Umzugsfirmen Appenzell - Umzugscheck.ch",
  "description": "Vergleichen Sie Umzugsfirmen im Appenzellerland. Kostenlose Offerten von geprüften lokalen Unternehmen.",
  "url": "https://umzugscheck.ch/appenzell",
  "areaServed": { "@type": "State", "name": "Appenzell", "containedInPlace": { "@type": "Country", "name": "Schweiz" } },
  "serviceType": "Umzugsservice"
};

export const Appenzell = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <OptimizedSEO title="Umzugsfirmen Appenzell – Vergleichen & bis zu 40% sparen" description="Vergleichen Sie Umzugsfirmen im Appenzellerland (AR & AI). Kostenlose Offerten von geprüften Unternehmen. ✓ Lokale Experten ✓ 100% gratis" canonicalUrl="https://umzugscheck.ch/appenzell" keywords="Umzug Appenzell, Umzugsfirma Herisau, Zügelfirma Appenzell, Umzugsunternehmen Appenzell" />
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Neue Buchung in Herisau</p><p className="text-xs text-muted-foreground">vor 9 Minuten</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Mountain className="h-3 w-3 mr-1" />Appenzellerland</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Umzugsfirmen im <span className="text-primary">Appenzell</span> vergleichen</h1>
              <p className="text-lg text-muted-foreground mb-6">Finden Sie die besten Umzugsfirmen im Appenzellerland. Lokale Experten für AR und AI.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} Personen vergleichen gerade im Appenzell</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">Von</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{appenzellCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">Nach</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{appenzellCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Top 10 Umzugsfirmen im Appenzell</h2>
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=appenzell"><Button variant="outline" size="lg">Alle Umzugsfirmen<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>

      <section className="py-12"><div className="container mx-auto px-4"><h2 className="text-2xl font-bold text-center mb-8">Umzugspreise im Appenzell</h2><div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">{priceExamples.map(p => <Card key={p.size} className="text-center"><CardContent className="p-6"><Home className="h-8 w-8 mx-auto mb-3 text-primary" /><h3 className="font-semibold mb-2">{p.size}</h3><p className="text-2xl font-bold text-primary">{p.avg}</p><p className="text-sm text-muted-foreground">{p.range}</p></CardContent></Card>)}</div></div></section>
      ...
      <section className="py-12 bg-muted/30"><div className="container mx-auto px-4 max-w-4xl"><h2 className="text-2xl font-bold mb-6">Umzug im Appenzell – Informationen</h2><div className="prose prose-gray max-w-none text-muted-foreground"><p className="mb-4">Das Appenzellerland besteht aus zwei Halbkantonen: Appenzell Ausserrhoden (AR) mit Herisau und Appenzell Innerrhoden (AI) mit Appenzell.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Hügelige Landschaft</h3><p className="mb-4">Die typische Hügellandschaft erfordert erfahrene Umzugsfirmen, die mit steilen Strassen und engen Zufahrten umgehen können.</p></div></div></section>
      ...
      <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold mb-4">Bereit für Ihren Umzug im Appenzell?</h2><p className="mb-6 opacity-90">Vergleichen Sie jetzt kostenlos die besten Umzugsfirmen</p><Link to="/umzug-offerte"><Button size="lg" variant="secondary" className="h-12 px-8">Jetzt vergleichen<ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div></section>
    </div>
  );
};

export default Appenzell;
