...
const topCompanies = [
  { name: "Berner Umzüge AG", rating: 4.9, reviews: 345, priceLevel: "fair", sponsored: true, available: true, badge: "Top bewertet" },
  { name: "Mittelland Moving", rating: 4.8, reviews: 287, priceLevel: "günstig", sponsored: true, available: true, badge: "Preistipp" },
  { name: "Bundesstadt Transporte", rating: 4.8, reviews: 234, priceLevel: "premium", sponsored: false, available: true, badge: "Premium" },
  { name: "Thun Express Umzüge", rating: 4.7, reviews: 198, priceLevel: "fair", sponsored: false, available: true, badge: "Beliebt" },
  { name: "Oberland Umzüge", rating: 4.7, reviews: 176, priceLevel: "günstig", sponsored: false, available: false, badge: null },
  { name: "Seeland Moving GmbH", rating: 4.6, reviews: 154, priceLevel: "fair", sponsored: false, available: true, badge: "Lokal" },
  { name: "Emmental Transporte", rating: 4.6, reviews: 132, priceLevel: "günstig", sponsored: false, available: true, badge: "Schnell" },
  { name: "Aare Umzugsteam", rating: 4.5, reviews: 118, priceLevel: "premium", sponsored: false, available: true, badge: null },
  { name: "Biel Umzüge", rating: 4.5, reviews: 98, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Köniz Express", rating: 4.4, reviews: 87, priceLevel: "günstig", sponsored: false, available: true, badge: null },
];
...
const usps = [
  { title: "Bundesstadt-Experten", desc: "Lokale Umzugsfirmen mit Bern-Kenntnis", icon: Castle },
  { title: "Bis 40% sparen", desc: "Vergleichen Sie Preise von Berner Firmen", icon: TrendingUp },
...
const faqs = [
  { question: "Was kostet ein Umzug in Bern?", answer: "Ein Umzug in Bern kostet zwischen CHF 490 und CHF 3'290 je nach Wohnungsgrösse. Die Preise in Bern entsprechen etwa dem Schweizer Durchschnitt." },
  { question: "Gibt es zweisprachige Umzugsfirmen?", answer: "Ja, im Raum Biel/Bienne und im Seeland gibt es mehrere zweisprachige Umzugsfirmen, die sowohl Deutsch als auch Französisch sprechen." },
  { question: "Wie buche ich einen Umzug in Bern?", answer: "Füllen Sie unser Formular in 2 Minuten aus und erhalten Sie bis zu 5 kostenlose Offerten von geprüften Berner Umzugsfirmen." },
  ...
  { question: "Gibt es Umzugsfirmen für das Berner Oberland?", answer: "Ja, mehrere unserer Partner sind auf Umzüge im Berner Oberland spezialisiert und kennen die besonderen Anforderungen der Region." },
];
    ...
    <div className="min-h-screen bg-background">
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Neue Buchung in Köniz</p><p className="text-xs text-muted-foreground">vor 4 Minuten</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Castle className="h-3 w-3 mr-1" />Bundesstadt</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Umzugsfirmen in <span className="text-primary">Bern</span> vergleichen</h1>
              <p className="text-lg text-muted-foreground mb-6">Finden Sie die besten Umzugsfirmen in Bern und Umgebung. Kostenlose Offerten von geprüften Partnern.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} Personen vergleichen gerade in Bern</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">Von</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{bernCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">Nach</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{bernCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=bern"><Button variant="outline" size="lg">Alle Umzugsfirmen<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>
