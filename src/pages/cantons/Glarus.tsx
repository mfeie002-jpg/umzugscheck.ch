...
const glarusCities = ["Glarus", "Näfels", "Netstal", "Niederurnen", "Oberurnen", "Mollis", "Ennenda", "Schwanden", "Elm", "Linthal"];

const topCompanies = [
  { name: "Glarner Umzüge AG", rating: 4.9, reviews: 92, priceLevel: "fair", sponsored: true, available: true, badge: "Top bewertet" },
  { name: "Linth Moving", rating: 4.8, reviews: 78, priceLevel: "günstig", sponsored: true, available: true, badge: "Preistipp" },
  { name: "Glarus Express", rating: 4.8, reviews: 64, priceLevel: "fair", sponsored: false, available: true, badge: "Lokal" },
  { name: "Näfels Transporte", rating: 4.7, reviews: 52, priceLevel: "günstig", sponsored: false, available: true, badge: null },
  { name: "Klöntal Umzüge", rating: 4.7, reviews: 46, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Schwanden Moving", rating: 4.6, reviews: 38, priceLevel: "günstig", sponsored: false, available: true, badge: null },
  { name: "Elm Zügeldienst", rating: 4.6, reviews: 32, priceLevel: "premium", sponsored: false, available: true, badge: "Bergexperte" },
  { name: "Netstal Transporte", rating: 4.5, reviews: 28, priceLevel: "fair", sponsored: false, available: true, badge: null },
  { name: "Linthal Umzüge", rating: 4.5, reviews: 24, priceLevel: "günstig", sponsored: false, available: false, badge: null },
  { name: "Glärnisch Moving", rating: 4.4, reviews: 18, priceLevel: "fair", sponsored: false, available: true, badge: null },
];

const priceExamples = [
  { size: "1-2 Zimmer", range: "CHF 420 - 790", avg: "CHF 580" },
  { size: "3-4 Zimmer", range: "CHF 880 - 1'520", avg: "CHF 1'100" },
  { size: "5+ Zimmer", range: "CHF 1'720 - 2'950", avg: "CHF 2'180" },
];
...
const usps = [
  { title: "Glarner Experten", desc: "Lokale Umzugsfirmen mit Regionenkenntnis", icon: Mountain },
  { title: "Bis 40% sparen", desc: "Vergleichen Sie Preise lokaler Glarner Firmen", icon: TrendingUp },
...
const faqs = [
  { question: "Was kostet ein Umzug in Glarus?", answer: "Ein Umzug in Glarus kostet zwischen CHF 420 und CHF 2'950 je nach Wohnungsgrösse." },
  { question: "Gibt es Spezialisten für Bergtäler?", answer: "Ja, mehrere Glarner Umzugsfirmen sind auf Umzüge in abgelegene Täler wie Elm und Linthal spezialisiert." },
  { question: "Wie buche ich einen Umzug in Glarus?", answer: "Füllen Sie unser Formular in 2 Minuten aus und erhalten Sie bis zu 5 kostenlose Offerten." },
  ...
  { question: "Bieten Sie auch Lagerung an?", answer: "Ja, viele unserer Partner bieten Einlagerungsmöglichkeiten in Glarus an." },
];

const jsonLd = {
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  "name": "Umzugsfirmen Glarus - Umzugscheck.ch",
  "description": "Vergleichen Sie Umzugsfirmen in Glarus. Kostenlose Offerten von geprüften lokalen Unternehmen.",
  "url": "https://umzugscheck.ch/glarus",
  "areaServed": { "@type": "State", "name": "Glarus", "containedInPlace": { "@type": "Country", "name": "Schweiz" } },
...
export const Glarus = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <OptimizedSEO title="Umzugsfirmen Glarus – Vergleichen & bis zu 40% sparen" description="Vergleichen Sie Umzugsfirmen in Glarus. Kostenlose Offerten von geprüften Unternehmen. ✓ Glarner Experten ✓ 100% gratis" canonicalUrl="https://umzugscheck.ch/glarus" keywords="Umzug Glarus, Umzugsfirma Glarus, Zügelfirma Glarus, Umzugsunternehmen Glarus" />
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Neue Buchung in Glarus</p><p className="text-xs text-muted-foreground">vor 10 Minuten</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Mountain className="h-3 w-3 mr-1" />Glarnerland</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Umzugsfirmen in <span className="text-primary">Glarus</span> vergleichen</h1>
              <p className="text-lg text-muted-foreground mb-6">Finden Sie die besten Umzugsfirmen in Glarus. Lokale Experten für das Glarnerland.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} Personen vergleichen gerade in Glarus</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">Von</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{glarusCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">Nach</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{glarusCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Top 10 Umzugsfirmen in Glarus</h2>
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=glarus"><Button variant="outline" size="lg">Alle Umzugsfirmen<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>

      <section className="py-12"><div className="container mx-auto px-4"><h2 className="text-2xl font-bold text-center mb-8">Umzugspreise in Glarus</h2><div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">{priceExamples.map(p => <Card key={p.size} className="text-center"><CardContent className="p-6"><Home className="h-8 w-8 mx-auto mb-3 text-primary" /><h3 className="font-semibold mb-2">{p.size}</h3><p className="text-2xl font-bold text-primary">{p.avg}</p><p className="text-sm text-muted-foreground">{p.range}</p></CardContent></Card>)}</div></div></section>
      ...
      <section className="py-12 bg-muted/30"><div className="container mx-auto px-4 max-w-4xl"><h2 className="text-2xl font-bold mb-6">Umzug in Glarus – Informationen</h2><div className="prose prose-gray max-w-none text-muted-foreground"><p className="mb-4">Der Kanton Glarus ist ein Bergkanton in der Ostschweiz mit dem Hauptort Glarus. Die einzigartige Landsgemeinde ist hier noch heute lebendig.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Bergtäler</h3><p className="mb-4">Umzüge in die Täler wie Elm oder Linthal erfordern erfahrene Umzugsfirmen mit entsprechender Ausrüstung.</p></div></div></section>
      ...
      <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold mb-4">Bereit für Ihren Umzug in Glarus?</h2><p className="mb-6 opacity-90">Vergleichen Sie jetzt kostenlos die besten Umzugsfirmen</p><Link to="/umzug-offerte"><Button size="lg" variant="secondary" className="h-12 px-8">Jetzt vergleichen<ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div></section>
    </div>
  );
};

export default Glarus;
