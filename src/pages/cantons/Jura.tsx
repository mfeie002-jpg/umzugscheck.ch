...
const juraCities = ["Delémont", "Porrentruy", "Saignelégier", "Bassecourt", "Courrendlin", "Courroux", "Courtételle", "Develier", "Fontenais", "Haute-Sorne"];

const topCompanies = [
  { name: "Jura Déménagement SA", rating: 4.9, reviews: 98, priceLevel: "fair", sponsored: true, available: true, badge: "Top noté" },
  { name: "Express Delémont", rating: 4.8, reviews: 82, priceLevel: "günstig", sponsored: true, available: true, badge: "Bon prix" },
  { name: "Porrentruy Moving", rating: 4.8, reviews: 68, priceLevel: "fair", sponsored: false, available: true, badge: "Local" },
  { name: "Franches-Montagnes Transport", rating: 4.7, reviews: 54, priceLevel: "günstig", sponsored: false, available: true, badge: null },
  { name: "Ajoie Déménagement", rating: 4.7, reviews: 48, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Saignelégier Express", rating: 4.6, reviews: 42, priceLevel: "premium", sponsored: false, available: true, badge: "Premium" },
  { name: "Bassecourt Transports", rating: 4.6, reviews: 36, priceLevel: "günstig", sponsored: false, available: true, badge: null },
  { name: "Courrendlin Moving", rating: 4.5, reviews: 28, priceLevel: "fair", sponsored: false, available: true, badge: null },
  { name: "Courroux Déménagement", rating: 4.5, reviews: 22, priceLevel: "günstig", sponsored: false, available: false, badge: null },
  { name: "Haute-Sorne Transport", rating: 4.4, reviews: 18, priceLevel: "fair", sponsored: false, available: true, badge: null },
];

const priceExamples = [
  { size: "1-2 pièces", range: "CHF 400 - 760", avg: "CHF 560" },
  { size: "3-4 pièces", range: "CHF 850 - 1'450", avg: "CHF 1'050" },
  { size: "5+ pièces", range: "CHF 1'650 - 2'800", avg: "CHF 2'100" },
];
...
const usps = [
  { title: "Experts jurassiens", desc: "Entreprises locales connaissant la région", icon: Mountain },
...
const faqs = [
  { question: "Combien coûte un déménagement dans le Jura?", answer: "Un déménagement dans le Jura coûte entre CHF 400 et CHF 2'800 selon la taille du logement. Le Jura est l'un des cantons les moins chers." },
  { question: "Y a-t-il des spécialistes pour les Franches-Montagnes?", answer: "Oui, plusieurs entreprises sont spécialisées pour Saignelégier et les Franches-Montagnes." },
  ...
  { question: "Proposez-vous du garde-meuble?", answer: "Oui, beaucoup de nos partenaires proposent des solutions de stockage dans le Jura." },
];

const jsonLd = {
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  "name": "Entreprises de déménagement Jura - Umzugscheck.ch",
  "description": "Comparez les entreprises de déménagement dans le Jura. Devis gratuits d'entreprises locales vérifiées.",
  "url": "https://umzugscheck.ch/jura",
  "areaServed": { "@type": "State", "name": "Jura", "containedInPlace": { "@type": "Country", "name": "Suisse" } },
...
export const Jura = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <OptimizedSEO title="Déménagement Jura – Comparez & économisez jusqu'à 40%" description="Comparez les entreprises de déménagement dans le Jura. Devis gratuits d'entreprises vérifiées. ✓ Experts locaux ✓ 100% gratuit" canonicalUrl="https://umzugscheck.ch/jura" keywords="Déménagement Jura, entreprise déménagement Delémont, déménageur Jura" />
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Nouvelle réservation à Delémont</p><p className="text-xs text-muted-foreground">il y a 14 minutes</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Mountain className="h-3 w-3 mr-1" />Canton du Jura</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Comparez les déménageurs dans le <span className="text-primary">Jura</span></h1>
              <p className="text-lg text-muted-foreground mb-6">Trouvez les meilleures entreprises de déménagement dans le Jura suisse.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} personnes comparent dans le Jura</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">De</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Choisir</option>{juraCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">À</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Choisir</option>{juraCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Top 10 déménageurs dans le Jura</h2>
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=jura"><Button variant="outline" size="lg">Tous les déménageurs<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>

      <section className="py-12"><div className="container mx-auto px-4"><h2 className="text-2xl font-bold text-center mb-8">Prix de déménagement dans le Jura</h2><div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">{priceExamples.map(p => <Card key={p.size} className="text-center"><CardContent className="p-6"><Home className="h-8 w-8 mx-auto mb-3 text-primary" /><h3 className="font-semibold mb-2">{p.size}</h3><p className="text-2xl font-bold text-primary">{p.avg}</p><p className="text-sm text-muted-foreground">{p.range}</p></CardContent></Card>)}</div></div></section>
      ...
      <section className="py-12 bg-muted/30"><div className="container mx-auto px-4 max-w-4xl"><h2 className="text-2xl font-bold mb-6">Déménagement dans le Jura – Informations</h2><div className="prose prose-gray max-w-none text-muted-foreground"><p className="mb-4">Le canton du Jura est le plus jeune canton suisse, créé en 1979. Delémont est la capitale et Porrentruy le chef-lieu de l'Ajoie.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Les Franches-Montagnes</h3><p className="mb-4">La région des Franches-Montagnes autour de Saignelégier est connue pour ses pâturages et chevaux. Les déménagements y sont généralement plus économiques.</p></div></div></section>
      ...
      <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold mb-4">Prêt pour votre déménagement dans le Jura?</h2><p className="mb-6 opacity-90">Comparez gratuitement les meilleures entreprises</p><Link to="/umzug-offerte"><Button size="lg" variant="secondary" className="h-12 px-8">Comparer maintenant<ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div></section>
    </div>
  );
};
