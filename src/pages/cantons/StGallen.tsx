...
import { Star, Shield, Clock, TrendingUp, CheckCircle, ArrowRight, MapPin, Users, Truck, Zap, ChevronRight, Building2, Home, Package, Church } from "lucide-react";

const stGallenCities = ["St. Gallen", "Rapperswil-Jona", "Wil", "Gossau", "Rorschach", "Buchs", "Uzwil", "Altstätten", "Flawil", "Wattwil"];

const topCompanies = [
  { name: "St. Galler Umzüge AG", rating: 4.9, reviews: 267, priceLevel: "fair", sponsored: true, available: true, badge: "Top bewertet" },
  { name: "Ostschweiz Moving", rating: 4.8, reviews: 234, priceLevel: "günstig", sponsored: true, available: true, badge: "Preistipp" },
  { name: "Bodensee Transporte", rating: 4.8, reviews: 198, priceLevel: "premium", sponsored: false, available: true, badge: "Premium" },
  { name: "Appenzell Express", rating: 4.7, reviews: 176, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Rheintal Umzüge", rating: 4.7, reviews: 154, priceLevel: "günstig", sponsored: false, available: true, badge: "Beliebt" },
  { name: "Toggenburg Moving", rating: 4.6, reviews: 132, priceLevel: "fair", sponsored: false, available: true, badge: "Lokal" },
  { name: "Wil Transporte GmbH", rating: 4.6, reviews: 118, priceLevel: "günstig", sponsored: false, available: true, badge: "Schnell" },
  { name: "Fürstenland Umzüge", rating: 4.5, reviews: 98, priceLevel: "premium", sponsored: false, available: false, badge: null },
  { name: "Gossau Express", rating: 4.5, reviews: 87, priceLevel: "fair", sponsored: false, available: true, badge: null },
  { name: "Rorschach Transporte", rating: 4.4, reviews: 76, priceLevel: "günstig", sponsored: false, available: true, badge: null },
];
...
const usps = [
  { title: "Ostschweiz-Experten", desc: "Lokale Umzugsfirmen mit Regionenkenntnis", icon: Church },
  { title: "Bis 40% sparen", desc: "Vergleichen Sie Preise von St. Galler Firmen", icon: TrendingUp },
...
const faqs = [
  { question: "Was kostet ein Umzug in St. Gallen?", answer: "Ein Umzug in St. Gallen kostet zwischen CHF 450 und CHF 3'090 je nach Wohnungsgrösse. Die Preise in der Ostschweiz liegen unter dem Schweizer Durchschnitt." },
  { question: "Gibt es Umzugsfirmen für das Rheintal?", answer: "Ja, mehrere unserer Partner sind im Rheintal und der Bodenseeregion tätig und kennen die besonderen Anforderungen der Region." },
  ...
  { question: "Bieten Sie auch Umzüge nach Österreich an?", answer: "Ja, einige unserer Partner führen grenzüberschreitende Umzüge nach Österreich und Liechtenstein durch." },
];
...
export const StGallen = () => {
  ...
  useEffect(() => { const i = setInterval(() => setActiveUsers(p => Math.max(5, p + Math.floor(Math.random() * 3) - 1)), 5000); return () => clearInterval(i); }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
      <OptimizedSEO title="Umzugsfirmen St. Gallen – Vergleichen & bis zu 40% sparen" description="Vergleichen Sie Umzugsfirmen in St. Gallen. Kostenlose Offerten von geprüften Unternehmen. ✓ Ostschweiz-Experten ✓ 100% gratis" canonicalUrl="https://umzugscheck.ch/stgallen" keywords="Umzug St. Gallen, Umzugsfirma St. Gallen, Zügelfirma St. Gallen, Umzugsunternehmen Ostschweiz" />
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Neue Buchung in Wil</p><p className="text-xs text-muted-foreground">vor 5 Minuten</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Church className="h-3 w-3 mr-1" />Ostschweiz</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Umzugsfirmen in <span className="text-primary">St. Gallen</span> vergleichen</h1>
              <p className="text-lg text-muted-foreground mb-6">Finden Sie die besten Umzugsfirmen in St. Gallen und der Ostschweiz. Kostenlose Offerten von geprüften Partnern.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} Personen vergleichen gerade in St. Gallen</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">Von</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{stGallenCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">Nach</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{stGallenCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Top 10 Umzugsfirmen in St. Gallen</h2>
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=stgallen"><Button variant="outline" size="lg">Alle Umzugsfirmen<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>

      <section className="py-12"><div className="container mx-auto px-4"><h2 className="text-2xl font-bold text-center mb-8">Umzugspreise in St. Gallen</h2><div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">{priceExamples.map(p => <Card key={p.size} className="text-center"><CardContent className="p-6"><Home className="h-8 w-8 mx-auto mb-3 text-primary" /><h3 className="font-semibold mb-2">{p.size}</h3><p className="text-2xl font-bold text-primary">{p.avg}</p><p className="text-sm text-muted-foreground">{p.range}</p></CardContent></Card>)}</div></div></section>
      ...
      <section className="py-12 bg-muted/30"><div className="container mx-auto px-4 max-w-4xl"><h2 className="text-2xl font-bold mb-6">Umzug in St. Gallen – Informationen</h2><div className="prose prose-gray max-w-none text-muted-foreground"><p className="mb-4">St. Gallen ist das kulturelle und wirtschaftliche Zentrum der Ostschweiz. Die Stadt ist bekannt für ihre Stiftsbibliothek und die textile Vergangenheit.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Umzüge in der Ostschweiz</h3><p className="mb-4">Die Ostschweiz bietet eine Vielzahl von Umzugsfirmen, die sowohl lokale als auch überregionale Umzüge durchführen. Viele sind auch auf grenzüberschreitende Umzüge spezialisiert.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Parkbewilligung</h3><p>Für Umzüge in St. Gallen empfehlen wir, eine temporäre Parkbewilligung bei der Stadt zu beantragen. Die Umzugsfirmen können Sie dabei unterstützen.</p></div></div></section>
      ...
      <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold mb-4">Bereit für Ihren Umzug in St. Gallen?</h2><p className="mb-6 opacity-90">Vergleichen Sie jetzt kostenlos die besten Umzugsfirmen</p><Link to="/umzug-offerte"><Button size="lg" variant="secondary" className="h-12 px-8">Jetzt vergleichen<ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div></section>
    </div>
  );
};
