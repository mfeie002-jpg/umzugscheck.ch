...
import { Star, Shield, Clock, TrendingUp, CheckCircle, ArrowRight, MapPin, Users, Truck, Zap, ChevronRight, Building2, Home, Package, Apple } from "lucide-react";

const thurgauCities = ["Frauenfeld", "Kreuzlingen", "Arbon", "Amriswil", "Weinfelden", "Romanshorn", "Sirnach", "Bischofszell", "Münchwilen", "Diessenhofen"];

const topCompanies = [
  { name: "Thurgauer Umzüge AG", rating: 4.9, reviews: 198, priceLevel: "fair", sponsored: true, available: true, badge: "Top bewertet" },
  { name: "Bodensee Moving", rating: 4.8, reviews: 176, priceLevel: "günstig", sponsored: true, available: true, badge: "Preistipp" },
  { name: "Kreuzlingen Express", rating: 4.8, reviews: 154, priceLevel: "premium", sponsored: false, available: true, badge: "Premium" },
  { name: "Oberthurgau Transporte", rating: 4.7, reviews: 132, priceLevel: "fair", sponsored: false, available: false, badge: null },
  { name: "Weinland Umzüge", rating: 4.7, reviews: 118, priceLevel: "günstig", sponsored: false, available: true, badge: "Beliebt" },
  { name: "Frauenfeld Moving GmbH", rating: 4.6, reviews: 98, priceLevel: "fair", sponsored: false, available: true, badge: "Lokal" },
  { name: "Seerücken Transporte", rating: 4.6, reviews: 87, priceLevel: "günstig", sponsored: false, available: true, badge: "Schnell" },
  { name: "Hinterthurgau Umzüge", rating: 4.5, reviews: 76, priceLevel: "premium", sponsored: false, available: false, badge: null },
  { name: "Arbon Express", rating: 4.5, reviews: 65, priceLevel: "fair", sponsored: false, available: true, badge: null },
  { name: "Untersee Transporte", rating: 4.4, reviews: 54, priceLevel: "günstig", sponsored: false, available: true, badge: null },
];

const priceExamples = [
  { size: "1-2 Zimmer", range: "CHF 420 - 790", avg: "CHF 590" },
  { size: "3-4 Zimmer", range: "CHF 890 - 1'490", avg: "CHF 1'090" },
  { size: "5+ Zimmer", range: "CHF 1'690 - 2'890", avg: "CHF 2'190" },
];
...
const usps = [
  { title: "Bodensee-Experten", desc: "Lokale Umzugsfirmen mit Regionenkenntnis", icon: Apple },
  { title: "Bis 40% sparen", desc: "Vergleichen Sie Preise von Thurgauer Firmen", icon: TrendingUp },
...
const faqs = [
  { question: "Was kostet ein Umzug im Thurgau?", answer: "Ein Umzug im Thurgau kostet zwischen CHF 420 und CHF 2'890 je nach Wohnungsgrösse. Der Thurgau gehört zu den günstigeren Kantonen." },
  { question: "Gibt es Umzugsfirmen für Umzüge nach Deutschland?", answer: "Ja, mehrere Thurgauer Umzugsfirmen führen grenzüberschreitende Umzüge nach Deutschland durch, besonders in die Bodenseeregion." },
  { question: "Wie buche ich einen Umzug im Thurgau?", answer: "Füllen Sie unser Formular in 2 Minuten aus und erhalten Sie bis zu 5 kostenlose Offerten von geprüften Thurgauer Umzugsfirmen." },
  ...
  { question: "Bieten Sie auch Lagerung an?", answer: "Ja, viele unserer Partner bieten Einlagerungsmöglichkeiten im Thurgau an." },
];

export const Thurgau = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <OptimizedSEO title="Umzugsfirmen Thurgau – Vergleichen & bis zu 40% sparen" description="Vergleichen Sie Umzugsfirmen im Thurgau. Kostenlose Offerten von geprüften Unternehmen. ✓ Bodensee-Experten ✓ 100% gratis" canonicalUrl="https://umzugscheck.ch/thurgau" keywords="Umzug Thurgau, Umzugsfirma Frauenfeld, Zügelfirma Kreuzlingen, Umzugsunternehmen Thurgau" />
        ...
        {showNotification && (
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="fixed bottom-4 right-4 z-50 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-medium">Neue Buchung in Kreuzlingen</p><p className="text-xs text-muted-foreground">vor 6 Minuten</p></div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Badge className="mb-4 bg-accent/10 text-accent border-accent/30"><Apple className="h-3 w-3 mr-1" />Bodensee-Region</Badge>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Umzugsfirmen im <span className="text-primary">Thurgau</span> vergleichen</h1>
              <p className="text-lg text-muted-foreground mb-6">Finden Sie die besten Umzugsfirmen im Thurgau. Günstige Preise in der Bodensee-Region.</p>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-2 h-2 bg-success rounded-full animate-pulse" /><span>{activeUsers} Personen vergleichen gerade im Thurgau</span></div>
            </motion.div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><Label className="text-sm">Von</Label><select value={fromCity} onChange={e => setFromCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{thurgauCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                      <div><Label className="text-sm">Nach</Label><select value={toCity} onChange={e => setToCity(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background text-sm"><option value="">Wählen</option>{thurgauCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    </div>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Top 10 Umzugsfirmen im Thurgau</h2>
          ...
          <div className="text-center mt-6"><Link to="/firmen?region=thurgau"><Button variant="outline" size="lg">Alle Umzugsfirmen<ChevronRight className="ml-2 h-4 w-4" /></Button></Link></div>
        </div>
      </section>

      <section className="py-12"><div className="container mx-auto px-4"><h2 className="text-2xl font-bold text-center mb-8">Umzugspreise im Thurgau</h2><div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">{priceExamples.map(p => <Card key={p.size} className="text-center"><CardContent className="p-6"><Home className="h-8 w-8 mx-auto mb-3 text-primary" /><h3 className="font-semibold mb-2">{p.size}</h3><p className="text-2xl font-bold text-primary">{p.avg}</p><p className="text-sm text-muted-foreground">{p.range}</p></CardContent></Card>)}</div></div></section>
      ...
      <section className="py-12 bg-muted/30"><div className="container mx-auto px-4 max-w-4xl"><h2 className="text-2xl font-bold mb-6">Umzug im Thurgau – Informationen</h2><div className="prose prose-gray max-w-none text-muted-foreground"><p className="mb-4">Der Thurgau liegt am Bodensee und ist bekannt für seinen Obstbau und seine ländliche Idylle. Der Kanton bietet attraktive Wohnlagen zu günstigen Preisen.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Günstige Umzugspreise</h3><p className="mb-4">Der Thurgau gehört zu den günstigsten Kantonen für Umzüge in der Deutschschweiz. Die ländliche Struktur und gute Erreichbarkeit halten die Kosten niedrig.</p><h3 className="text-lg font-semibold mt-6 mb-3 text-foreground">Grenznahe Umzüge</h3><p>Viele Thurgauer Umzugsfirmen führen auch Umzüge in die deutsche Bodenseeregion durch.</p></div></div></section>
      ...
      <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold mb-4">Bereit für Ihren Umzug im Thurgau?</h2><p className="mb-6 opacity-90">Vergleichen Sie jetzt kostenlos die besten Umzugsfirmen</p><Link to="/umzug-offerte"><Button size="lg" variant="secondary" className="h-12 px-8">Jetzt vergleichen<ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div></section>
    </div>
  );
};

export default Thurgau;
