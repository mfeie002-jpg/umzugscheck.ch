...
const HERO_BG = "https://images.unsplash.com/photo-1549451371-64aa98a6f660?auto=format&fit=crop&w=2000&q=80";
...
const companies = [
  { id: "basler-umzug", name: "Basler Umzug AG", rating: 4.9, reviewCount: 198, badges: ["Top Bewertung", "Lokal"], priceLevel: "Mittel", isPopular: true, responseTime: "< 2h" },
  { id: "rhein-transporte", name: "Rhein Transporte", rating: 4.7, reviewCount: 156, badges: ["Preis-Sieger"], priceLevel: "Günstig", isBestPrice: true, responseTime: "< 1h" },
  { id: "dreiland-umzuege", name: "Dreiland Umzüge", rating: 4.8, reviewCount: 134, badges: ["International"], priceLevel: "Premium", isPremium: true, responseTime: "< 3h" },
];

const priceExamples = [
  { size: "1.5 - 2.5 Zimmer", price: "CHF 600 – 1'000", subtext: "Studio / kleine Wohnung", icon: Home, savings: "bis CHF 400" },
  { size: "3.5 - 4.5 Zimmer", price: "CHF 880 – 1'400", subtext: "Familienwohnung", icon: Building2, savings: "bis CHF 560" },
  { size: "5+ Zimmer / Haus", price: "ab CHF 1'700", subtext: "Villa / Einfamilienhaus", icon: Home, savings: "bis CHF 680" },
];
...
const services = [
  ...
  { title: "Einlagerung", icon: Warehouse, description: "Sichere Lagerräume in Basel", link: "/services/lagerung" },
...
const testimonials = [
  { name: "Andrea W.", location: "Basel → Riehen", rating: 5, text: "Perfekter Service! Die Basler Firma kannte alle Tricks für enge Altstadtgassen. Sehr empfehlenswert!", date: "vor 3 Tagen", verified: true, savedAmount: 380 },
  { name: "Michael S.", location: "Allschwil → Basel", rating: 5, text: "Grenzüberschreitender Umzug von Deutschland war kein Problem. Professionell und pünktlich.", date: "vor 5 Tagen", verified: true, savedAmount: 450 },
  { name: "Claudia H.", location: "St. Johann → Gundeldingen", rating: 5, text: "Unkompliziert und fair. CHF 350 gespart gegenüber dem ersten Angebot!", date: "vor 1 Woche", verified: true, savedAmount: 350 },
];
...
const faqs = [
  { question: "Was kostet ein Umzug in Basel?", answer: "Ein 2-Zimmer-Umzug innerhalb Basel kostet ca. CHF 600-1'000, ein 4-Zimmer-Umzug ca. CHF 880-1'400. Preise können bei grenzüberschreitenden Umzügen variieren." },
  { question: "Kann ich auch aus Deutschland nach Basel umziehen?", answer: "Ja! Viele unserer Partner sind auf internationale Umzüge spezialisiert. Sie kennen alle Zollformalitäten im Dreiländereck." },
  { question: "Brauche ich eine Bewilligung in der Basler Altstadt?", answer: "Ja, besonders in der Innenstadt und am Spalenberg sind Halteverbotszonen nötig. Kosten ca. CHF 70-120." },
  { question: "Wie früh sollte ich in Basel buchen?", answer: "Wir empfehlen 4-6 Wochen Vorlauf. Während der Fasnacht und bei Messen ist frühere Buchung ratsam." },
...
const nearbyMunicipalities = [
  { name: "Umzug Riehen", href: "/riehen/umzugsfirmen" },
  { name: "Umzug Allschwil", href: "/allschwil/umzugsfirmen" },
  { name: "Umzug Binningen", href: "/binningen/umzugsfirmen" },
  { name: "Umzug Muttenz", href: "/muttenz/umzugsfirmen" },
  { name: "Umzug Pratteln", href: "/pratteln/umzugsfirmen" },
];
...
const QuickPriceEstimate = ({ size }: { size: string }) => {
  if (!size) return null;
  const estimates: Record<string, { min: number; max: number; savings: number }> = { "1-2": { min: 600, max: 1000, savings: 400 }, "2.5-3": { min: 750, max: 1200, savings: 480 }, "3.5-4": { min: 880, max: 1400, savings: 560 }, "4.5-5": { min: 1200, max: 1800, savings: 620 }, "5+": { min: 1700, max: 2800, savings: 680 } };
...
const WhatsAppButton = () => <motion.a href="https://wa.me/41791234567?text=Hallo,%20ich%20möchte%20eine%20Umzugsofferte%20für%20Basel" target="_blank" rel="noopener noreferrer" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2, type: "spring" }} className="fixed bottom-24 right-4 lg:bottom-6 lg:right-6 z-40 w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all group"><MessageCircle className="w-7 h-7 text-white" /></motion.a>;

const BaselLandingPage = () => {
  ...
  const [selectedCanton, setSelectedCanton] = useState("basel");
  const [activeTestimonial, setActiveTestimonial] = useState(0);

  useEffect(() => { const i = setInterval(() => setLiveViewers(p => Math.max(15, Math.min(35, p + Math.floor(Math.random() * 7) - 3))), 3500); return () => clearInterval(i); }, []);
  ...
  const handleFormSubmit = (e: React.FormEvent) => { e.preventDefault(); localStorage.setItem("uc_prefill", JSON.stringify({ from: fromLocation, to: toLocation, size: apartmentSize, source: "basel-landing", timestamp: Date.now() })); navigate("/umzugsofferten"); };
  const handleCantonChange = (value: string) => { setSelectedCanton(value); const canton = cantons.find(c => c.value === value); if (canton && value !== "basel") navigate(canton.href); };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
          ...
          <div className="container relative z-10 px-4 py-12 md:py-16">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center lg:text-left">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-5 leading-tight">Umzug Basel – <br className="hidden md:block" /><span className="text-secondary">Jetzt gratis vergleichen</span></h1>
                <p className="text-lg md:text-xl text-white/85 mb-6 max-w-xl mx-auto lg:mx-0">Vergleichen Sie <strong>40+ geprüfte Umzugsfirmen</strong> in Basel-Stadt, Baselland & Dreiländereck.<span className="text-green-400 font-semibold"> Bis zu 40% sparen.</span></p>
                <div className="grid grid-cols-3 gap-4 mb-8 max-w-lg mx-auto lg:mx-0"><div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={2678} suffix="+" /></div><div className="text-xs text-white/60">Bewertungen</div></div><div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={40} suffix="+" /></div><div className="text-xs text-white/60">Firmen</div></div><div className="text-2xl md:text-3xl font-bold text-amber-400 flex items-center gap-1"><Star className="w-6 h-6 fill-amber-400" />4.8<span className="text-xs text-white/60 font-normal ml-1">/ 5</span></div></div>
        ...
        <section className="py-14 md:py-18 bg-gradient-to-br from-blue-50/50 to-slate-50/50"><div className="container px-4"><div className="max-w-4xl mx-auto"><motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><Badge variant="outline" className="mb-4"><MapPin className="w-3 h-3 mr-1" />Lokales Wissen</Badge><h2 className="text-2xl md:text-3xl font-bold mb-6">Umziehen in der Region Basel</h2><div className="prose prose-slate max-w-none"><p className="text-muted-foreground leading-relaxed mb-4">Basel liegt im Dreiländereck Schweiz-Deutschland-Frankreich und ist bekannt für seine Pharma- und Kulturszene. Die Stadt zieht internationale Fachkräfte an, was zu vielen grenzüberschreitenden Umzügen führt. Beliebte Wohnquartiere sind <strong>Gundeldingen, St. Johann, Kleinbasel</strong> und die Gemeinden <strong>Riehen</strong> und <strong>Allschwil</strong>.</p><p className="text-muted-foreground leading-relaxed">Besonderheiten beim Umziehen in Basel: Die historische Altstadt hat enge Gassen, viele Expats ziehen aus Deutschland oder Frankreich zu, und während der Fasnacht sind Umzüge praktisch unmöglich. Unsere Partner kennen alle lokalen Gegebenheiten.</p></div></motion.div></div></div></section>
