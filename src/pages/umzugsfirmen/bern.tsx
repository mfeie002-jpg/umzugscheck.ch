...
const HERO_BG = "https://images.unsplash.com/photo-1587974928442-77dc3e0dba72?auto=format&fit=crop&w=2000&q=80";
...
const companies = [
  { id: "berner-umzug", name: "Berner Umzug AG", rating: 4.9, reviewCount: 234, badges: ["Top Bewertung", "Lokal"], priceLevel: "Mittel", isPopular: true, responseTime: "< 2h" },
  { id: "aare-transporte", name: "Aare Transporte", rating: 4.7, reviewCount: 178, badges: ["Preis-Sieger"], priceLevel: "Günstig", isBestPrice: true, responseTime: "< 1h" },
  { id: "hauptstadt-umzuege", name: "Hauptstadt Umzüge", rating: 4.8, reviewCount: 156, badges: ["Premium"], priceLevel: "Premium", isPremium: true, responseTime: "< 3h" },
];

const priceExamples = [
  { size: "1.5 - 2.5 Zimmer", price: "CHF 580 – 950", subtext: "Studio / kleine Wohnung", icon: Home, savings: "bis CHF 380" },
  { size: "3.5 - 4.5 Zimmer", price: "CHF 820 – 1'300", subtext: "Familienwohnung", icon: Building2, savings: "bis CHF 520" },
  { size: "5+ Zimmer / Haus", price: "ab CHF 1'600", subtext: "Villa / Einfamilienhaus", icon: Home, savings: "bis CHF 640" },
];
...
const services = [
  ...
  { title: "Einlagerung", icon: Warehouse, description: "Sichere Lagerräume in Bern", link: "/services/lagerung" },
...
const testimonials = [
  { name: "Hans M.", location: "Bern → Thun", rating: 5, text: "Hervorragender Service! Die Berner Firma war super freundlich und hat alles perfekt organisiert.", date: "vor 2 Tagen", verified: true, savedAmount: 420 },
  { name: "Sabine K.", location: "Köniz → Bern", rating: 5, text: "Unkompliziert und günstig. Der Vergleich hat sich wirklich gelohnt – CHF 350 gespart!", date: "vor 4 Tagen", verified: true, savedAmount: 350 },
  { name: "Rolf B.", location: "Muri → Ostermundigen", rating: 5, text: "Professionelle Abwicklung, pünktlich und vorsichtig mit den Möbeln. Top!", date: "vor 1 Woche", verified: true, savedAmount: 300 },
];
...
const faqs = [
  { question: "Was kostet ein Umzug in Bern?", answer: "Ein 2-Zimmer-Umzug innerhalb Bern kostet ca. CHF 580-950, ein 4-Zimmer-Umzug ca. CHF 820-1'300. Die Preise sind etwas günstiger als in Zürich." },
  { question: "Brauche ich eine Bewilligung in der Berner Altstadt?", answer: "Ja, in der UNESCO-geschützten Altstadt ist eine Sonderbewilligung nötig. Unsere Partner kennen die Vorschriften und helfen bei der Beantragung." },
  { question: "Wie früh sollte ich in Bern buchen?", answer: "Wir empfehlen 4-6 Wochen Vorlauf. Zur Hochsaison oder bei Semesterwechsel (Uni Bern) sind 6-8 Wochen ratsam." },
  { question: "Gibt es besondere Herausforderungen in Bern?", answer: "Die Berner Altstadt mit ihren Laubengängen und engen Gassen erfordert erfahrene lokale Umzugsfirmen. Auch die Hügellagen können herausfordernd sein." },
...
const nearbyMunicipalities = [
  { name: "Umzug Thun", href: "/thun/umzugsfirmen" },
  { name: "Umzug Köniz", href: "/koeniz/umzugsfirmen" },
  { name: "Umzug Biel", href: "/biel/umzugsfirmen" },
  { name: "Umzug Ostermundigen", href: "/ostermundigen/umzugsfirmen" },
  { name: "Umzug Muri bei Bern", href: "/muri/umzugsfirmen" },
];
...
const QuickPriceEstimate = ({ size }: { size: string }) => {
  if (!size) return null;
  const estimates: Record<string, { min: number; max: number; savings: number }> = { "1-2": { min: 580, max: 950, savings: 380 }, "2.5-3": { min: 700, max: 1100, savings: 440 }, "3.5-4": { min: 820, max: 1300, savings: 520 }, "4.5-5": { min: 1150, max: 1700, savings: 580 }, "5+": { min: 1600, max: 2700, savings: 640 } };
...
const WhatsAppButton = () => <motion.a href="https://wa.me/41791234567?text=Hallo,%20ich%20möchte%20eine%20Umzugsofferte%20für%20Bern" target="_blank" rel="noopener noreferrer" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2, type: "spring" }} className="fixed bottom-24 right-4 lg:bottom-6 lg:right-6 z-40 w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all group"><MessageCircle className="w-7 h-7 text-white" /></motion.a>;

const BernLandingPage = () => {
  ...
  const [selectedCanton, setSelectedCanton] = useState("bern");
  const [activeTestimonial, setActiveTestimonial] = useState(0);

  useEffect(() => { const i = setInterval(() => setLiveViewers(p => Math.max(20, Math.min(45, p + Math.floor(Math.random() * 7) - 3))), 3500); return () => clearInterval(i); }, []);
  ...
  const handleFormSubmit = (e: React.FormEvent) => { e.preventDefault(); localStorage.setItem("uc_prefill", JSON.stringify({ from: fromLocation, to: toLocation, size: apartmentSize, source: "bern-landing", timestamp: Date.now() })); navigate("/umzugsofferten"); };
  const handleCantonChange = (value: string) => { setSelectedCanton(value); const canton = cantons.find(c => c.value === value); if (canton && value !== "bern") navigate(canton.href); };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
          ...
          <div className="container relative z-10 px-4 py-12 md:py-16">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center lg:text-left">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-5 leading-tight">Umzug Bern – <br className="hidden md:block" /><span className="text-secondary">Jetzt gratis vergleichen</span></h1>
                <p className="text-lg md:text-xl text-white/85 mb-6 max-w-xl mx-auto lg:mx-0">Vergleichen Sie <strong>50+ geprüfte Umzugsfirmen</strong> in Bern, Thun & Umgebung.<span className="text-green-400 font-semibold"> Bis zu 40% sparen.</span></p>
                <div className="grid grid-cols-3 gap-4 mb-8 max-w-lg mx-auto lg:mx-0"><div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={3456} suffix="+" /></div><div className="text-xs text-white/60">Bewertungen</div></div><div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={50} suffix="+" /></div><div className="text-xs text-white/60">Firmen</div></div><div className="text-2xl md:text-3xl font-bold text-amber-400 flex items-center gap-1"><Star className="w-6 h-6 fill-amber-400" />4.8<span className="text-xs text-white/60 font-normal ml-1">/ 5</span></div></div>
        ...
        <section className="py-14 md:py-18 bg-gradient-to-br from-blue-50/50 to-slate-50/50"><div className="container px-4"><div className="max-w-4xl mx-auto"><motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><Badge variant="outline" className="mb-4"><MapPin className="w-3 h-3 mr-1" />Lokales Wissen</Badge><h2 className="text-2xl md:text-3xl font-bold mb-6">Umziehen im Kanton Bern</h2><div className="prose prose-slate max-w-none"><p className="text-muted-foreground leading-relaxed mb-4">Der Kanton Bern ist der zweitgrösste Kanton der Schweiz und beherbergt die Bundeshauptstadt. Die Stadt Bern mit ihrer UNESCO-geschützten Altstadt ist bekannt für ihre historischen Laubengänge und die malerische Lage an der Aare. Beliebte Wohngebiete sind <strong>Länggasse, Kirchenfeld, Bümpliz</strong> und <strong>Muri bei Bern</strong>.</p><p className="text-muted-foreground leading-relaxed">Besonderheiten beim Umziehen in Bern: Die Altstadt erfordert Sonderbewilligungen, viele Gebäude haben enge Treppenhäuser, und die Hügellagen wie Kirchenfeld können für Umzugsfahrzeuge herausfordernd sein. Lokale Firmen kennen diese Gegebenheiten.</p></div></motion.div></div></div></section>
