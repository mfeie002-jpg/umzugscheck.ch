...
const CANTON_NAME = "Graubünden";
const CANTON_SLUG = "graubuenden";
const HERO_BG = "https://images.unsplash.com/photo-1531366936337-7c912a4589a7?auto=format&fit=crop&w=2000&q=80";

const companies = [
  { id: "buendner-umzuege", name: "Bündner Umzüge AG", rating: 4.9, reviewCount: 134, isPopular: true },
  { id: "chur-transporte", name: "Chur Transporte", rating: 4.7, reviewCount: 98, isBestPrice: true },
  { id: "davos-moving", name: "Davos Moving", rating: 4.8, reviewCount: 76, isPremium: true },
];

const priceExamples = [
  { size: "1.5 - 2.5 Zimmer", price: "CHF 550 – 950", icon: Home, savings: "bis CHF 380" },
  { size: "3.5 - 4.5 Zimmer", price: "CHF 850 – 1'350", icon: Building2, savings: "bis CHF 540" },
  { size: "5+ Zimmer", price: "ab CHF 1'500", icon: Home, savings: "bis CHF 600" },
];

const faqs = [
  { question: "Was kostet ein Umzug in Graubünden?", answer: "Die Kosten variieren je nach Distanz und Zugänglichkeit. Bergregionen können teurer sein." },
  { question: "Bieten Firmen auch Umzüge in Bergdörfer an?", answer: "Ja! Unsere Partner sind auf alpine Umzüge spezialisiert." },
];
...
export default function GraubuendenLandingPage() {
  ...
  useEffect(() => { const i = setInterval(() => setLiveViewers(p => Math.max(6, Math.min(20, p + Math.floor(Math.random() * 7) - 3))), 3500); return () => clearInterval(i); }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        <section className="relative min-h-[92vh] flex items-center overflow-hidden">
          ...
          <div className="container relative z-10 px-4 py-12"><div className="grid lg:grid-cols-2 gap-8 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center lg:text-left">
              ...
              <p className="text-lg text-white/85 mb-6">Vergleichen Sie <strong>15+ geprüfte Umzugsfirmen</strong> in Chur, Davos & St. Moritz.<span className="text-green-400 font-semibold"> Bis zu 40% sparen.</span></p>
              <div className="grid grid-cols-3 gap-4 mb-8 max-w-lg mx-auto lg:mx-0"><div><div className="text-2xl font-bold text-white"><AnimatedCounter end={987} suffix="+" /></div><div className="text-xs text-white/60">Bewertungen</div></div><div><div className="text-2xl font-bold text-white"><AnimatedCounter end={15} suffix="+" /></div><div className="text-xs text-white/60">Firmen</div></div><div className="text-2xl font-bold text-amber-400 flex items-center gap-1"><Star className="w-6 h-6 fill-amber-400" />4.8</div></div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="hidden lg:block">
              <Card className="bg-white/[0.98] backdrop-blur-xl border-0 shadow-2xl rounded-2xl"><CardContent className="p-8">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  <div><Label>Von (PLZ oder Ort)</Label><Input placeholder="z.B. 7000 Chur" value={fromLocation} onChange={(e) => setFromLocation(e.target.value)} className="mt-1.5 h-12" /></div>
                  <div><Label>Nach (PLZ oder Ort)</Label><Input placeholder="z.B. 7270 Davos" value={toLocation} onChange={(e) => setToLocation(e.target.value)} className="mt-1.5 h-12" /></div>
        ...
        <section className="py-6 bg-muted/50 border-y"><div className="container px-4"><div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center"><div><div className="text-2xl font-bold text-primary">15+</div><div className="text-xs text-muted-foreground">Geprüfte Firmen</div></div><div><div className="text-2xl font-bold text-primary">980+</div><div className="text-xs text-muted-foreground">Bewertungen</div></div><div><div className="text-2xl font-bold text-primary">4.8★</div><div className="text-xs text-muted-foreground">Durchschnitt</div></div><div><div className="text-2xl font-bold text-green-600">40%</div><div className="text-xs text-muted-foreground">Ersparnis</div></div></div></div></section>

        <section className="py-12 bg-muted/30"><div className="container px-4"><div className="text-center mb-10"><Badge variant="secondary" className="mb-3">Top Firmen</Badge><h2 className="text-2xl font-bold">Beste Umzugsfirmen in {CANTON_NAME}</h2></div><div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">{companies.map((c) => (<Card key={c.id} className="h-full"><CardContent className="p-6"><div className="flex justify-between mb-4"><div><h3 className="font-bold">{c.name}</h3><div className="flex items-center gap-1 mt-1"><Star className="w-4 h-4 fill-amber-400 text-amber-400" /><span>{c.rating}</span><span className="text-muted-foreground text-sm">({c.reviewCount})</span></div></div>{c.isPopular && <Badge className="bg-primary">Beliebt</Badge>}{c.isBestPrice && <Badge className="bg-green-600">Preis-Tipp</Badge>}</div></CardContent></Card>))}</div></div></section>
        ...
        <section className="py-12"><div className="container px-4"><div className="max-w-3xl mx-auto"><h2 className="text-2xl font-bold mb-6">Umzugsfirmen im Kanton {CANTON_NAME}</h2><p className="text-muted-foreground mb-6">Graubünden ist der grösste Kanton der Schweiz mit Chur als Hauptstadt. Umzüge in Bergregionen erfordern spezialisierte Firmen.</p><div className="mt-6 flex flex-wrap gap-2"><Link to="/umzugsfirmen/stgallen"><Badge variant="outline">St. Gallen</Badge></Link><Link to="/umzugsfirmen/tessin"><Badge variant="outline">Tessin</Badge></Link><Link to="/umzugsfirmen/uri"><Badge variant="outline">Uri</Badge></Link></div></div></div></section>
