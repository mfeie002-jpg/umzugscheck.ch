            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="hidden lg:block">
              <Card className="bg-white/[0.98] backdrop-blur-xl border-0 shadow-2xl rounded-2xl"><CardContent className="p-8">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  <div><Label>Von (PLZ oder Ort)</Label><Input placeholder="z.B. 1950 Sion" value={fromLocation} onChange={(e) => setFromLocation(e.target.value)} className="mt-1.5 h-12" /></div>
                  <div><Label>Nach (PLZ oder Ort)</Label><Input placeholder="z.B. 3900 Brig" value={toLocation} onChange={(e) => setToLocation(e.target.value)} className="mt-1.5 h-12" /></div>
