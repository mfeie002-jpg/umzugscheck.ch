...
const HERO_BG = "https://images.unsplash.com/photo-1515488764276-beab7607c1e6?auto=format&fit=crop&w=2000&q=80";
...
const companies = [
  { id: "zuercher-umzug", name: "Zürcher Umzug AG", rating: 4.9, reviewCount: 312, badges: ["Top Bewertung", "Lokal"], priceLevel: "Mittel", isPopular: true, responseTime: "< 2h" },
  ...
  { id: "city-move-zh", name: "City Move Zürich", rating: 4.7, reviewCount: 189, badges: ["Premium"], priceLevel: "Premium", isPremium: true, responseTime: "< 3h" },
];

const priceExamples = [
  { size: "1.5 - 2.5 Zimmer", price: "CHF 650 – 1'100", subtext: "Studio / kleine Wohnung", icon: Home, savings: "bis CHF 440" },
  { size: "3.5 - 4.5 Zimmer", price: "CHF 950 – 1'500", subtext: "Familienwohnung", icon: Building2, savings: "bis CHF 600" },
  { size: "5+ Zimmer / Haus", price: "ab CHF 1'800", subtext: "Villa / Einfamilienhaus", icon: Home, savings: "bis CHF 720" },
];
...
const services = [
  ...
  { title: "Einlagerung", icon: Warehouse, description: "Sichere Lagerräume in Zürich", link: "/services/lagerung" },
...
const testimonials = [
  { name: "Peter M.", location: "Zürich → Winterthur", rating: 5, text: "Fantastischer Service! Die Firma war pünktlich, professionell und CHF 500 günstiger als andere Angebote.", date: "vor 2 Tagen", verified: true, savedAmount: 500 },
  { name: "Lisa B.", location: "Oerlikon → Altstetten", rating: 5, text: "Umzug in Zürich kann stressig sein, aber mit der richtigen Firma war alles perfekt organisiert.", date: "vor 5 Tagen", verified: true, savedAmount: 380 },
  { name: "Marco R.", location: "Seefeld → Wiedikon", rating: 5, text: "Sehr zufrieden! Schnelle Antworten, faire Preise und sorgfältiger Umgang mit meinen Möbeln.", date: "vor 1 Woche", verified: true, savedAmount: 420 },
];
...
const faqs = [
  { question: "Was kostet ein Umzug in Zürich?", answer: "Ein 2-Zimmer-Umzug innerhalb Zürich kostet ca. CHF 650-1'100, ein 4-Zimmer-Umzug ca. CHF 950-1'500. Zürich ist teurer als ländliche Regionen wegen Parkgebühren und engeren Strassen." },
  { question: "Brauche ich eine Parkbewilligung in Zürich?", answer: "Ja, in den meisten Quartieren ist eine Halteverbotszone nötig. Kosten: ca. CHF 80-150. Die meisten unserer Partner übernehmen die Beantragung." },
  { question: "Wie früh sollte ich in Zürich buchen?", answer: "Mindestens 4-6 Wochen vorher. Zum Monatsende und in der Hochsaison (April-September) sind 8-10 Wochen empfohlen." },
  { question: "Gibt es Aufzüge-Probleme in Zürcher Altbauten?", answer: "Ja, viele Altbauten haben keinen Lift. Möbellifte sind dann unverzichtbar. Unsere Partner beraten Sie kostenlos zur besten Lösung." },
...
const WhatsAppButton = () => (
  <motion.a href="https://wa.me/41791234567?text=Hallo,%20ich%20möchte%20eine%20Umzugsofferte%20für%20Zürich" target="_blank" rel="noopener noreferrer"
...
const ZuerichLandingPage = () => {
  ...
  const [liveViewers, setLiveViewers] = useState(34);
  ...
  useEffect(() => { const i = setInterval(() => setLiveViewers(p => Math.max(25, Math.min(50, p + Math.floor(Math.random() * 7) - 3))), 3500); return () => clearInterval(i); }, []);
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem("uc_prefill", JSON.stringify({ from: fromLocation, to: toLocation, size: apartmentSize, source: "zuerich-landing", timestamp: Date.now() }));
  ...
  const schemaOrg = {
    ...
    "@graph": [
      { "@type": "LocalBusiness", "name": "Umzugscheck.ch - Kanton Zürich", "description": "Vergleiche 80+ geprüfte Umzugsfirmen im Kanton Zürich.", "areaServed": { "@type": "Place", "name": "Kanton Zürich, Schweiz" }, "priceRange": "CHF 650 - CHF 4000", "aggregateRating": { "@type": "AggregateRating", "ratingValue": "4.8", "reviewCount": "5234" } },
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <html lang="de-CH" />
        <title>Umzug Zürich – Jetzt gratis Umzugsfirma finden | Bis 40% sparen</title>
        <meta name="description" content="Vergleiche 80+ geprüfte Umzugsfirmen im Kanton Zürich ✓ Kostenlose Offerten ✓ Zürich, Winterthur, Uster ✓ Bis zu 40% sparen!" />
      ...
      <main>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center lg:text-left">
                <div className="inline-flex items-center gap-2 mb-4">
                  <Select value={selectedCanton} onValueChange={handleCantonChange}>
                    ...
                    <SelectContent className="bg-card border-border">{cantons.map((c) => <SelectItem key={c.value} value={c.value}>{c.label}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-5 leading-tight">Umzug Zürich – <br className="hidden md:block" /><span className="text-secondary">Jetzt gratis vergleichen</span></h1>
                <p className="text-lg md:text-xl text-white/85 mb-6 max-w-xl mx-auto lg:mx-0">Vergleichen Sie <strong>80+ geprüfte Umzugsfirmen</strong> in Zürich, Winterthur & Umgebung.<span className="text-green-400 font-semibold"> Bis zu 40% sparen.</span></p>
                <div className="grid grid-cols-3 gap-4 mb-8 max-w-lg mx-auto lg:mx-0">
                  <div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={5234} suffix="+" /></div><div className="text-xs text-white/60">Bewertungen</div></div>
                  <div className="text-center lg:text-left"><div className="text-2xl md:text-3xl font-bold text-white"><AnimatedCounter end={80} suffix="+" /></div><div className="text-xs text-white/60">Firmen</div></div>
                ...
                <Card className="bg-white/[0.98] backdrop-blur-xl border-0 shadow-2xl rounded-2xl overflow-hidden">
                  <CardContent className="p-6 md:p-8">
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-4 mt-4">
                      ...
                      <div><Label htmlFor="to" className="text-sm font-medium">Umzug nach</Label><Input id="to" placeholder="z.B. 8400 oder Winterthur" value={toLocation} onChange={(e) => setToLocation(e.target.value)} className="mt-1.5 h-12" /></div>
                      <div><Label htmlFor="size" className="text-sm font-medium">Wohnungsgrösse</Label><Select value={apartmentSize} onValueChange={setApartmentSize}><SelectTrigger className="mt-1.5 h-12"><SelectValue placeholder="Bitte wählen..." /></SelectTrigger><SelectContent className="bg-card border-border"><SelectItem value="1-2">1-2 Zimmer</SelectItem><SelectItem value="2.5-3">2.5-3 Zimmer</SelectItem><SelectItem value="3.5-4">3.5-4 Zimmer</SelectItem><SelectItem value="4.5-5">4.5-5 Zimmer</SelectItem><SelectItem value="5+">5+ Zimmer / Haus</SelectItem></SelectContent></Select></div>
                      <QuickPriceEstimate size={apartmentSize} />
                      <Button type="submit" size="lg" className="w-full bg-secondary hover:bg-secondary/90 text-secondary-foreground font-bold text-lg py-6 shadow-[0_4px_20px_rgba(220,38,38,0.4)] group">Jetzt Offerten anfordern<ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" /></Button>
                    </form>
                    <div className="flex flex-wrap justify-center gap-4 mt-6 pt-4 border-t border-border">{["Kostenlos", "Unverbindlich", "Geprüfte Firmen"].map((item) => <div key={item} className="flex items-center gap-1.5 text-sm text-muted-foreground"><Check className="w-4 h-4 text-green-500" />{item}</div>)}</div>
                    <div className="flex items-center justify-center gap-1.5 mt-4 text-xs text-muted-foreground"><Lock className="w-3 h-3" />Schweizer Hosting · SSL verschlüsselt</div>
                  </CardContent>
                </Card>
              </motion.div>
            </div>
        ...
        <section className="py-6 bg-muted/40 border-y border-border">
          <div className="container px-4">
            <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
              <div className="flex items-center gap-2"><div className="flex">{[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 text-amber-400 fill-amber-400" />)}</div><span className="text-sm font-bold">4.8/5</span><span className="text-xs text-muted-foreground">(<AnimatedCounter end={5234} /> Bewertungen)</span></div>
              <div className="hidden md:block h-6 w-px bg-border" />
              <div className="flex items-center gap-2 text-sm"><Users className="w-4 h-4 text-primary" /><span className="font-bold"><AnimatedCounter end={80} />+</span><span className="text-muted-foreground">Firmen in Zürich</span></div>
              <div className="hidden md:block h-6 w-px bg-border" />
              <div className="flex items-center gap-2 text-sm"><Award className="w-4 h-4 text-primary" /><span className="font-bold"><AnimatedCounter end={28000} />+</span><span className="text-muted-foreground">vermittelte Umzüge</span></div>
              <div className="hidden md:block h-6 w-px bg-border" />
              <div className="flex items-center gap-2 text-sm"><TrendingDown className="w-4 h-4 text-green-600" /><span className="font-bold text-green-600">Ø 35%</span><span className="text-muted-foreground">Ersparnis</span></div>
            </div>
          </div>
        </section>
        ...
        <section className="py-14 md:py-18">
          <div className="container px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
              <Badge variant="outline" className="mb-4"><Star className="w-3 h-3 mr-1 fill-amber-400 text-amber-400" />Kundenstimmen</Badge>
              <h2 className="text-3xl md:text-4xl font-bold mb-3">Das sagen unsere Kunden</h2>
              <p className="text-muted-foreground">Über 5'200 zufriedene Kunden im Kanton Zürich</p>
            </motion.div>
            <div className="max-w-2xl mx-auto">
              <AnimatePresence mode="wait">
                <motion.div key={activeTestimonial} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-card border border-border rounded-2xl p-6">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><span className="text-lg font-bold text-primary">{testimonials[activeTestimonial].name[0]}</span></div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1"><span className="font-semibold">{testimonials[activeTestimonial].name}</span>{testimonials[activeTestimonial].verified && <BadgeCheck className="w-4 h-4 text-blue-500" />}</div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3"><MapPin className="w-3 h-3" />{testimonials[activeTestimonial].location}<span>•</span>{testimonials[activeTestimonial].date}</div>
                      <div className="flex mb-3">{[...Array(testimonials[activeTestimonial].rating)].map((_, i) => <Star key={i} className="w-4 h-4 text-amber-400 fill-amber-400" />)}</div>
                      <p className="text-muted-foreground italic">"{testimonials[activeTestimonial].text}"</p>
                      {testimonials[activeTestimonial].savedAmount && <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium"><TrendingDown className="w-3.5 h-3.5" />CHF {testimonials[activeTestimonial].savedAmount} gespart</div>}
                    </div>
                  </div>
                </motion.div>
              </AnimatePresence>
              <div className="flex justify-center gap-2 mt-4">{testimonials.map((_, idx) => <button key={idx} onClick={() => setActiveTestimonial(idx)} className={`w-2 h-2 rounded-full transition-colors ${idx === activeTestimonial ? 'bg-primary' : 'bg-border'}`} />)}</div>
            </div>
          </div>
        </section>
        ...
        <section className="py-14 md:py-18 bg-muted/20">
          <div className="container px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
              <Badge variant="secondary" className="mb-4"><Crown className="w-3 h-3 mr-1" />Unsere Top-Partner</Badge>
              <h2 className="text-3xl md:text-4xl font-bold mb-3">Top Umzugsfirmen im Kanton Zürich</h2>
            ...
            <div className="grid md:grid-cols-3 gap-5 max-w-4xl mx-auto">
              {companies.map((company, index) => (
                <motion.div key={company.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
            ...
            <div className="text-center mt-10"><Button asChild size="lg" className="bg-secondary hover:bg-secondary/90 text-secondary-foreground"><Link to="/umzugsofferten">Alle 80+ Firmen vergleichen<ArrowRight className="ml-2 w-5 h-5" /></Link></Button></div>
          </div>
        </section>
        ...
        <section className="py-14 md:py-18">
          <div className="container px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
              <Badge variant="outline" className="mb-4"><Banknote className="w-3 h-3 mr-1" />Preisübersicht</Badge>
              <h2 className="text-3xl md:text-4xl font-bold mb-3">Was kostet ein Umzug in Zürich?</h2>
        ...
        <section className="py-14 md:py-18">
          <div className="container px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
              <Badge variant="secondary" className="mb-4"><ShieldCheck className="w-3 h-3 mr-1" />Unsere Garantien</Badge>
        ...
        <section className="py-16 md:py-24 bg-gradient-to-br from-primary via-primary to-primary/90 text-primary-foreground relative overflow-hidden">
          ...
          <div className="container px-4 text-center relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <p className="text-lg text-primary-foreground/85 mb-8 max-w-xl mx-auto">80+ geprüfte Firmen · Bis 40% sparen · 100% gratis</p>
              <Button asChild size="lg" className="bg-white text-primary hover:bg-white/90 font-bold text-lg px-8 py-6 shadow-xl"><Link to="/umzugsofferten">Jetzt Offerten anfordern<ArrowRight className="ml-2 w-5 h-5" /></Link></Button>
              <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-sm text-primary-foreground/80">{["Keine versteckten Kosten", "100% unverbindlich", "Schweizer Datenschutz"].map((item) => <div key={item} className="flex items-center gap-2"><Check className="w-4 h-4" />{item}</div>)}</div>
            </motion.div>
          </div>
        </section>
      </main>
