-- Create function to get canton from postal code
-- This helps match leads to providers based on location
...
-- Create function to count leads for a provider in current month
...
-- Create function to find matching providers for a lead
...
-- Create function to auto-assign providers to new leads
...
-- Create trigger to automatically assign providers when lead is created
DROP TRIGGER IF EXISTS trigger_auto_assign_providers ON public.leads;
CREATE TRIGGER trigger_auto_assign_providers
  BEFORE INSERT ON public.leads
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_assign_providers_to_lead();

-- Add comment for documentation
COMMENT ON FUNCTION public.auto_assign_providers_to_lead() IS 
'Automatically assigns matching service providers to new leads based on location, availability, and preferences';

COMMENT ON FUNCTION public.find_matching_providers(text, text, numeric) IS 
'Finds service providers that match lead criteria: serves the canton, is approved/active, has capacity, meets min job value';

COMMENT ON FUNCTION public.count_provider_leads_this_month(uuid) IS 
'Counts how many leads have been assigned to a provider in the current month';

COMMENT ON FUNCTION public.get_canton_from_postal(text) IS 
'Maps Swiss postal code to canton abbreviation for lead matching';
