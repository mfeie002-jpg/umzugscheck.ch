...
function activate(context) {
    ...
    initializeSupabase();
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('umzugscheck.nextCodexTask', () => fetchNextTask('codex')), vscode.commands.registerCommand('umzugscheck.nextCopilotTask', () => fetchNextTask('copilot')), vscode.commands.registerCommand('umzugscheck.markDone', markTaskDone), vscode.commands.registerCommand('umzugscheck.showQueue', showTaskQueue), vscode.commands.registerCommand('umzugscheck.generateTasks', generateTasks));
...
function initializeSupabase() {
    ...
    const url = config.get('supabaseUrl') || process.env.SUPABASE_URL;
    const key = config.get('supabaseAnonKey') || process.env.SUPABASE_ANON_KEY;
    ...
    if (finalUrl && finalKey) {
        supabase = (0, supabase_js_1.createClient)(finalUrl, finalKey);
        console.log('Supabase client initialized');
    }
    else {
        vscode.window.showWarningMessage('Umzugscheck AI Agent: Supabase not configured. Go to Settings to configure.');
    }
}
    ...
    try {
        ...
        if (count && count > 0) {
            ...
            if (config.get('showNotifications')) {
                const action = await vscode.window.showInformationMessage(`🤖 ${count} AI Task(s) pending!`, 'Load CODEX', 'Load COPILOT', 'Dismiss');
...
async function fetchNextTask(agent) {
    if (!supabase) {
    ...
    try {
        ...
        const panel = vscode.window.createWebviewPanel('aiTask', `${agent.toUpperCase()} Task: ${task.title}`, vscode.ViewColumn.Beside, { enableScripts: true });
        panel.webview.html = getTaskWebviewContent(task, formattedPrompt);
        // Copy to clipboard
        await vscode.env.clipboard.writeText(formattedPrompt);
        // Show notification with action
        const action = await vscode.window.showInformationMessage(`📋 ${agent.toUpperCase()} Task loaded and copied! Open Copilot Chat (Cmd+I) and paste.`, 'Open Copilot Chat', 'Mark Done');
...
function formatPromptForCopilot(task) {
...
function getTaskWebviewContent(task, formattedPrompt) {
...
async function markTaskDone() {
    ...
    try {
        ...
        const config = vscode.workspace.getConfiguration('umzugscheck');
        if (config.get('autoLoadNextTask')) {
            const action = await vscode.window.showInformationMessage('🔄 Load next task?', `Next ${previousAgent.toUpperCase()}`, 'Show Queue', 'Done');
...
async function generateTasks() {
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (error)
                throw error;
            vscode.window.showInformationMessage(`✅ ${data.tasks_created} new tasks created! ${data.summary || ''}`, 'Show Queue').then(action => {
